...
## 价值主张设计（一分钟版本）
### 1.加值宣言
* 旅行轨迹APP是一款针对于旅行爱好者的软件，拥有照片打卡与地标识别这两大核心功能。作为旅游爱好者想要通过打卡的方式来记录自己的旅行轨迹，那么可以通过用户上传的照片，进行照片信息的读取，通过**逆地理解析API**获取照片经纬度，再调用**静态地图API**获取位置，实现照片打卡功能。（重要）用户在影视/图片中看到心仪的景点，则可上传图片（影片截图），软件将通过调用**百度——地标识别API**，反馈给用户有关该图片中的地标名称。识别地标名称与查询相应的信息，帮助用户解决不知道图片中出现的地标名称的问题，提供图片地标查询的功能。（重要）用户想要旅游又不知道去哪里好，可以根据用户浏览记录来为用户智能推荐景点，也可以根据用户所在位置智能推荐周边景点。（次重要-暂不做）
