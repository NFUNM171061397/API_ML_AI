...
* 作为旅游爱好者想要通过打卡的方式来记录自己的旅行轨迹，那么可以通过用户上传的照片，进行照片信息的读取，通过**逆地理解析API**获取照片经纬度，再调用**静态地图API**获取位置，实现照片打卡功能。（重要）
* 用户在影视/图片中看到心仪的景点，则可上传图片（影片截图），软件将通过调用**百度——地标识别API**，反馈给用户有关该图片中的地标名称。识别地标名称与查询相应的信息，帮助用户解决不知道图片中出现的地标名称的问题，提供图片地标查询的功能。（重要）
* 用户想要旅游又不知道去哪里好，可以根据用户浏览记录来为用户智能推荐景点，也可以根据用户所在位置智能推荐周边景点。（次重要-暂不做）


### 2.核心价值（最小可行性产品）
1. 照片打卡：对用户上传的照片进行信息读取，通过逆地理解析API获取照片经纬度，再通过调用静态地图API，根据经纬度获取地图位置。
2. 地标识别：用户从手机相册上传图片/拍摄照片，通过调用地标识别API，结合百度百科资料，返回图中的地标名称与信息。
...
| 问题 | 结果| 功能|人工智能API| 优先级|
| ------  | ------- |----|----|----|
| 用户想要快速知道图片中的地标名称是什么？ |  上传图片，即能识别该地标名称  |地标识别|地标识别API|非常重要|
| 用户想要一款有趣的旅行记录app |  上传照片，实现地图打卡，生成专属于你的旅行轨迹  |拍照打卡|逆地理解析API、静态地图API|非常重要|
| 用户想要发现世界上更多美好的景点？ | 推荐系统通过浏览记录智能筛选您可能感兴趣的内容    |智能推荐|基于推荐系统|次重要|
...
* 地标识别模块：用户可以查看地标分类，浏览智能推荐的景点，也可以从手机相册上传图片/拍摄照片。用户点击图标，上传单张图片，操作页面将调用地标识别API，结合百度百科资料，在结果页面上返回图中的地标名称与信息。
* 附近去处模块：用户可以浏览系统智能推荐的附近去处。
...
总结：通过资料收集与实践，当前产品所拥有的功能都是当前可实现的技术。
...
#### 三家API对比
从成熟度来看，市场上，百度地图、腾讯地图、高德地图，这三家都是比较成熟的。从性价比来看，百度之前的优点就是没有调用次数限制，这点比腾讯的10000/天好多了，但是现在都有次数限定了，所以高德的性价比比较高。再从实际的应用上来看，在逆地理编码API中，用经纬度查询实际地理位置时，百度地图只能精确到小数点后第2位，只能够定位到某个区，但是高德地图，能够精确到小数点后第4位。因此适合于本产品的API我认为是高德地图API。

### 3.使用后风险报告
* 逆地理解析API与静态地图API都属于web服务API。这类API正在广泛应用中，无论是在生活中，线上还是线下。网络制图和地理空间信息在线使用在过去几十年中发展迅速。世界上几乎每个人都使用地图信息，不管一个人是否意识到这一点。现在几乎每部手机都有定位服务， 地球上的每一个事件和物体都有一个位置。由于因特网的发展， 这一地理空间位置数据的使用迅速扩大。大量的地理空间数据可以在网上获得，并用于网络应用程序和地图，以便查看、分析、建模和模拟。如今, Web服务引入了更广泛的功能，仍在扩展，以纳入不断增长的应用程序需求，并具有能够共享内容的优势和分析网络地图数据的能力。
* [web服务API的发展资料](https://zhuanlan.zhihu.com/p/65674651)
* web服务API市场竞争程度，就拿市场上，百度地图、腾讯地图、高德地图，这三家地图平台来看。目前，手机地图市场已经进入成熟期，活跃用户数基本达到稳定状态，人口红利消失，用户增长缓慢。同时，手机地图app同质化现象也存在，如何借助大数据和AI技术拉开差距，成为地图服务商的必经之路。所以市场竞争仍然很激烈，主要就是在大数据和AI之间的创新了。
* [中国手机地图行业市场现状和竞争格局分析](https://www.sohu.com/a/343283030_161117)
* 百度的地标识别API是刚推出不久的API，还在不断的完善中，收录了近12万的地标景点。虽然是刚推出不久的API，但比起其他家的，已经略胜一筹了，微软的计算机影像API中名人与地标识别，只收录了全世界9000个自然和人工地标，因此百度的地标识别API是它的将近12倍。虽然有12万地标，但这还是远远不够的，仍会有许多的图像无法识别，无法输出，当前百度大脑仍在学习中，相信在未来，它的发展会很好，会被人们广泛运用。

### 4.加分项
使用复杂度：用了2个以上机器学习与人工智能的API之输入及输出
* [拍照打卡-逆地理解析API](http://nfunm171061397.gitee.io/photo_python)
* [拍照打卡-静态地图API](http://nfunm171061397.gitee.io/photo_python)
